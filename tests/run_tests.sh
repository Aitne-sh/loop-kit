...
echo "== init projects the 13 Codex-native skills with explicit invocation policy =="
...
echo "== discard: non-TTY needs a choice; keep cancels an unstarted plan but preserves manual new/ =="
...
echo "== discard: a live orchestration stops its supervisor and attributed worker before queue removal =="
...
echo "== discard rollback: SAFE reviewer produces one inverse commit without rewriting history =="
...
echo "== discard rollback: dispatched net-zero history still requires independent side-effect review =="
...
echo "== discard rollback: a manual/parallel product commit makes rollback unavailable but still cancels =="
...
echo "== discard rollback: a claimed manual dependent is parked and makes rollback unavailable =="
...
echo "== discard: resuming a discard-parked manual dependent never scraps kept work =="
...
echo "== discard rollback: reviewer UNSAFE retains changes, returns 4, and removes the plan =="
...
echo "== discard: a pending request blocks run/add/resume/stop; unlock remains the recovery boundary =="
...
echo "== discard: a fixed durable choice cannot be reversed on retry =="
...
echo "== discard: manual tasks with a cancelled planned dependency are parked explicitly =="
...
echo "== discard: a partially stamped legacy authority converges without overwriting conflicts =="
...
echo "== discard: untracked worker files quarantine the worktree instead of deleting it =="
...
echo "== fleet clean --orphans: a quarantined discard worktree needs --force, and names it =="
...
echo "== discard: a parked discard-peer recovers with an executable resume hint =="
...
echo "== discard: corrupted task metadata cannot delete another clean worktree =="
...
echo "== discard: ignored user files also quarantine a worker (managed ignored files do not) =="
...
echo "== discard rollback: parent ignored data blocks an overlapping inverse before review =="
